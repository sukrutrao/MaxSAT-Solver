 ...
 * This solver uses Sequential Encoding and performs a linear search from unsatisfied to satisfied
 * 
 * Sukrut Rao
 * CS15BTECH11036
 * 
...
class MaxSATSolver
{
    private:
        ...
        vec<Var> relaxation_variables; // Minisat vector to store the relaxation variables for each clause 
        vec<Var> assumption_variables; // Minisat vector to store the assumption variables for each relaxation 
        vec<Lit> assumptions; // Minisat vector to store the assumption literals
        vec< vec<Var> > encoded_variables; // Vec of Vecs to store the encoding variables for sequential encoding
        RetVal solve_with_relaxation(int); // to solve the formula by relaxing a fixed number of clauses
        void add_encoded_clauses(int); // to add encoded clauses for relaxation to the formula
...
void MaxSATSolver::initialize()
{
    ...
    // the number of relaxation variables and assumption variables are up to the number of clauses
    ...
    {
    	relaxation_variables[i] = solver.newVar(); // add each relaxation variable to the solver
    }
...
RetVal MaxSATSolver::solve_with_relaxation(int relaxed_clauses_count)
{
	...
	/*
	 * set all previous assumptions to false to nullify all past relaxation constraints
	 */
...
void MaxSATSolver::add_encoded_clauses(int relaxed_clauses_count)
{
	...
	// get the equivalent literal with positive polarity for use, with negation when required
	...
	for(int i = 0; i < clause_count-2; i++)
	{
		for(int j = 0; j < relaxed_clauses_count; j++)
		{
			...
			/*
			 * impose the clause that for i in range 1 to n-2 and for j in range 1 to k
			 * ai_j -> ai+1_j
			 */
			...
			if(j < relaxed_clauses_count-1)
			{
				...
				/*
				 * impose the clause that for i in range 1 to n-2 and j in range 1 to k-1
				 * (ai_j and yj+1) -> ai+1_j+1
				 */
 ...
 * Arguments : number_of_clauses_satisfied - the maximum number of clauses that could be satisfied 
 */
